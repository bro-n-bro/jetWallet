                ...
                <div class="list" v-if="searchResult.length">
                    <div class="item" v-for="(validator, index) in searchResult" :key="index">
                        <div class="validator_wrap">
                            <div class="validator">
                                <div class="logo">
                                    <img :src="`https://raw.githubusercontent.com/cosmostation/chainlist/main/chain/${store.networks[store.currentNetwork].prefix}/moniker/${validator.operator_address}.png`" alt="" loading="lazy">
                                </div>
    ...
    emitter.on('search', ({ query, source }) => {
        if (source === 'validators') {
            ...
            for (let key in validators.value) {
                if (validators.value[key].moniker.toLocaleLowerCase().includes(query.toLocaleLowerCase())) {
