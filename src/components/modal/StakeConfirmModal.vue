<template>
    <!-- Stake confirm modal -->
    <section class="page_container inner_page_container stake_confirm" :class="{ closing: isClosing }">
        <!-- Loader -->
...
<script setup>
    ...
    function closeHandler() {
        ...
        setTimeout(() => {
            ...
            emitter.emit('close_stake_confirm_modal')
        }, 200)
    }
    ...
    async function calcVotingPower() {
        try {
            ...
            console.error(`Components/Modal/StakeConfirmModal.vue: ${error.message}`)
        }
    }
